...
var solrDoc = require('./document');
querystring = require('querystring'), http = require("http"),
    _ = require("underscore"), logger = require('nlogger').logger(module);
;
...
var solrClient = function(init_opts) {
    ...
    if (typeof client == 'object') {
      ...
      if (typeof client.proxy == 'object') {
        if (typeof client.proxy.host != "undefined"
          ...
          var temp_opts = client;
          _.extends(client,
              {
                host : temp_opts.proxy.host,
                port : temp_opts.proxy.port,
                path : temp_opts.host + ':' + temp_opts.port + '/'
                    + temp_opts.path,
                headers : {
                  Host : temp_opts.host
                }
      ...
      _.extend(defaultSolrClient, client);
    }
  ...
  this.addDoc = function(doc, commit, callback, context) {
    if (doc instanceof solrDoc) {
      var _client = this._client;
      _client.method = 'POST';
      if (typeof _client['core'] == 'string') {
        _client.path += '/' + _client['core'];
