...
var solrDoc = require('./document'), querystring = require('querystring'), http = require("http"), _ = require("underscore"), logger = require(
    ...
    if (typeof client == 'object') {
      ...
      if (typeof client.proxy == 'object') {
        if (typeof client.proxy.host != "undefined"
          ...
          var temp_opts = _.clone(client);
          _.extend(client, {
            host : temp_opts.proxy.host,
            port : temp_opts.proxy.port,
            path : (temp_opts.port == 443 ? 'https://' : 'http://')
                + temp_opts.host + ':' + temp_opts.port + temp_opts.path
          });
          delete client.proxy;
        }
      }
